...
class Player(wavelink.Player):
    ...
    async def teardown(self):
        ...
        try:
            await self.destroy()
            await self.ctx_glob2.send(":raised_hand:")
    ...
    async def add_spot_tracks_track(self, ctx, tracks):
        ...
        if self.is_playing and self.queue.lenght > 1:
            ...
            self.q_msg = await ctx.send(embed=embed)
...
class Music(commands.Cog, wavelink.WavelinkMixin):
        ...
        if not member.bot and after.channel is None:
            ...
            if not [m for m in before.channel.members if not m.bot]:
                if self.get_player(member.guild).is_connected:
                    await member.guild.text_channels[0].send("Sesli sohbette kimse olmadığı için ÇIKIŞ YAPIYORUM.")
    ...
    async def next_command(self, ctx):
        ...
        if not player.queue.upcoming and player.queue.autoplay == Autoplay.OFF:
            raise NoMoreTracks
...
class BoşKomutlar(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
